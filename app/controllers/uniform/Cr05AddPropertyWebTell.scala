...
class Cr05AddPropertyWebTell(govUkSummaryList: GovukSummaryList) extends GenericWebTell[(Cr05AddProperty, PropertyType, Option[Int]), Html]:

  /**
    * Not edit links.
    */
  def confirmationSummary(in: Cr05AddProperty, messages: UniformMessages[Html]): SummaryList =
    val sum = summaryList(in, PropertyType.EXISTING, None, messages) // Not important, links are removed
    SummaryList(sum.rows.map(x => x.copy(actions = None)), classes = "govuk-!-margin-bottom-9")

  private def formatUrl(journey: String, propertyType: PropertyType, index: Option[Int]) =
    index.map { x =>
  ...
  override def render(in: (Cr05AddProperty, PropertyType, Option[Int]), key: String, messages: UniformMessages[Html]): Html =
    govUkSummaryList(summaryList(in._1, in._2, in._3, messages))
