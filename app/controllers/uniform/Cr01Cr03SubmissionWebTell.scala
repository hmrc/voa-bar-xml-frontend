...
class Cr01Cr03SubmissionWebTell(govUkSumaryList: GovukSummaryList) extends GenericWebTell[Cr01Cr03Submission, Html] {
  ...
  private def reasonSummaryList(in: Cr01Cr03Submission, messages: UniformMessages[Html]): Seq[SummaryListRow] = {
    val reasonReport =
      SummaryListRow(
        ...
        actions = Some(Actions(items =
          Seq(
            ActionItem(controllers.routes.ReportReasonController.onPageLoad.url, HtmlContent(messages("check-answers.changeLabel")))
          )
        ))
      )
    ...
    val removalReason = in.removalReason.map { rr =>
      SummaryListRow(
        ...
        actions = Some(Actions(items =
          Seq(
            ActionItem(controllers.routes.UniformController.myJourney("why-should-it-be-removed").url, HtmlContent(messages("check-answers.changeLabel")))
          )
        ))
      )
    }
    ...
    val otherReason = in.otherReason.map { or =>
      SummaryListRow(
        ...
        actions = Some(Actions(items =
          Seq(
            ActionItem(controllers.routes.UniformController.myJourney("other-reason").url, HtmlContent(messages("check-answers.changeLabel")))
          )
        ))
      )
    }
  ...
  def summaryList(in: Cr01Cr03Submission, messages: UniformMessages[Html]): SummaryList = {
    ...
    val baReport = SummaryListRow(
      ...
      actions = Some(Actions(items =
        Seq(
          ActionItem(controllers.routes.UniformController.myJourney("ba-report").url, HtmlContent(messages("check-answers.changeLabel")))
        )
      ))
    )
    val baRef    = SummaryListRow(
      ...
      actions = Some(Actions(items =
        Seq(
          ActionItem(controllers.routes.UniformController.myJourney("ba-ref").url, HtmlContent(messages("check-answers.changeLabel")))
        )
      ))
    )
    ...
    val uprn = SummaryListRow(
      ...
      actions = Some(Actions(items =
        Seq(
          ActionItem(controllers.routes.UniformController.myJourney("UPRN").url, HtmlContent(messages("check-answers.changeLabel")))
        )
      ))
    )
    ...
    val propertyAddress = SummaryListRow(
      ...
      actions = Some(Actions(items =
        Seq(
          ActionItem(controllers.routes.UniformController.myJourney("property-address").url, HtmlContent(messages("check-answers.changeLabel")))
        )
      ))
    )
    ...
    val contactDetails      = SummaryListRow(
      ...
      actions = Some(Actions(items =
        Seq(
          ActionItem(controllers.routes.UniformController.myJourney("property-contact-details").url, HtmlContent(messages("check-answers.changeLabel")))
        )
      ))
    )
    val sameAddressQuestion = SummaryListRow(
      ...
      actions = Some(Actions(items =
        Seq(
          ActionItem(controllers.routes.UniformController.myJourney("same-contact-address").url, HtmlContent(messages("check-answers.changeLabel")))
        )
      ))
    )
    ...
    val contactAddress = in.contactAddress.map { contactAddress =>
      ...
      SummaryListRow(
        ...
        actions = Some(Actions(items =
          Seq(
            ActionItem(controllers.routes.UniformController.myJourney("contact-address").url, HtmlContent(messages("check-answers.changeLabel")))
          )
        ))
      )
    }
    ...
    val effectiveDate = SummaryListRow(
      ...
      actions = Some(Actions(items =
        Seq(
          ActionItem(controllers.routes.UniformController.myJourney("effective-date").url, HtmlContent(messages("check-answers.changeLabel")))
        )
      ))
    )
    ...
    val havePlanningRef = SummaryListRow(
      ...
      actions = Some(Actions(items =
        Seq(
          ActionItem(controllers.routes.UniformController.myJourney("have-planning-ref").url, HtmlContent(messages("check-answers.changeLabel")))
        )
      ))
    )
    ...
    val planningRef = in.planningRef.map { planningRef =>
      SummaryListRow(
        ...
        actions = Some(Actions(items =
          Seq(
            ActionItem(controllers.routes.UniformController.myJourney("planning-ref").url, HtmlContent(messages("check-answers.changeLabel")))
          )
        ))
      )
    }
    ...
    val noPlanningRef = in.noPlanningReference.map { noPlanningRef =>
      SummaryListRow(
        ...
        actions = Some(Actions(items =
          Seq(
            ActionItem(controllers.routes.UniformController.myJourney("why-no-planning-ref").url, HtmlContent(messages("check-answers.changeLabel")))
          )
        ))
      )
    }
    ...
    val comments =
      SummaryListRow(
        ...
        actions = Some(Actions(items =
          Seq(
            ActionItem(controllers.routes.UniformController.myJourney("comments").url, HtmlContent(messages("check-answers.changeLabel")))
          )
        ))
      )
