...
class CouncilTaxUploadController @Inject() (
  ...
  private[controllers] def fileUploadDetails(username: String, password: String)(using request: OptionalDataRequest[?])
  ...
  private[controllers] def loadPage(expectedPage: (String, InitiateResponse) => Result)(using request: OptionalDataRequest[?]): Future[Result] =
    (for
    ...
    implicit request =>
      val formWithError                                                  =
        if showEmptyError then
          form.withError("file", messagesApi("error.file.empty"))
        else
          form
  ...
  def onError(reference: String): Action[JsValue] = getData.async(parse.tolerantJson) {
    implicit request =>
      logger.warn(s"Unable to upload XML, error detail: ${request.body}")
      (for
  ...
  private def saveReportStatus(login: Login, reference: String)(using request: Request[?]): Future[Either[Result, Unit]] =
