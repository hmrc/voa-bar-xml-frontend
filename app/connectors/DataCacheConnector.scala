...
class DataCacheConnectorImpl @Inject() (val sessionRepository: SessionRepository, val cascadeUpsert: CascadeUpsert)(using ec: ExecutionContext)
  ...
  def getEntryByField[A](field: String, value: String, key: String)(using fmt: Format[A]): Future[Option[A]] =
    sessionRepository.getByField(field, value).map { optionalCacheMap =>
      optionalCacheMap.flatMap(cacheMap => cacheMap.getEntry(key))
    }
  ...
  def removeFromCollection[A](cacheId: String, collectionKey: String, item: A)(using fmt: Format[A]): Future[CacheMap] =
    sessionRepository.get(cacheId).flatMap { optionalCacheMap =>
      optionalCacheMap.fold(throw Exception(s"Couldn't find document with key $cacheId")) { cacheMap =>
        val newSeq      = cacheMap.data(collectionKey).as[Seq[A]].filterNot(x => x == item)
        val newCacheMap = if newSeq.isEmpty then
