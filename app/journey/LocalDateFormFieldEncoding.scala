...
object LocalDateFormFieldEncoding:
  val year: List[String]  = List("year")
  val month: List[String] = List("month")
  val day: List[String]   = List("day")
...
class LocalDateFormFieldEncoding extends FormFieldEncoding[LocalDate]:
  ...
  private val testRegex             = """\d+""".r.pattern
  private val earliestEffectiveDate = LocalDate.of(1993, 4, 1)

  private def validateDateField(day: String, month: String, year: String): Either[ErrorTree, LocalDate] =
    for
      day       <- validateDay(day)
      ...
      finalDate <-
        Try(
          ...
          if date.isBefore(earliestEffectiveDate) || date.isAfter(LocalDate.now()) then
  ...
  private def validateDay(day: String): Either[ErrorTree, Int] =
    if testRegex.matcher(day).matches() then
      val intDay = day.toInt
      if intDay > 31 || intDay < 1 then Left(ErrorTree.one(NonEmptyList.one(ErrorMsg("error.day.range")).append(ErrorMsg("day"))))
      else Right(intDay)
    else
      Left(ErrorTree.one(NonEmptyList.one(ErrorMsg("error.day.number")).append(ErrorMsg("day"))))

  private def validateMont(month: String): Either[ErrorTree, Int] =
    if testRegex.matcher(month).matches() then
      val intMonth = month.toInt
      if intMonth > 12 | intMonth < 1 then Left(ErrorTree.one(NonEmptyList.one(ErrorMsg("error.month.range")).append(ErrorMsg("month"))))
      else Right(intMonth)
    else
      Left(ErrorTree.one(NonEmptyList.one(ErrorMsg("error.month.number")).append(ErrorMsg("month"))))

  private def validateYear(year: String): Either[ErrorTree, Int] =
    if testRegex.matcher(year).matches() then
      val intYear = year.toInt
      if intYear < 1993 || intYear > LocalDate.now().getYear then Left(ErrorTree.one(NonEmptyList.one(ErrorMsg("error.year.range")).append(ErrorMsg("year"))))
      else Right(intYear)
    else
      Left(ErrorTree.one(NonEmptyList.one(ErrorMsg("error.year.number")).append(ErrorMsg("year"))))
